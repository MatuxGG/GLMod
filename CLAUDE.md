...
### Exceptions
- **End-user translations**: strings served by the Good Loss API and consumed by `TranslationService` are localized by design — they are user-facing translations and must NOT be touched here.
...
### 3.2 `ITranslationService` — [Interfaces/ITranslationService.cs](GLMod/Services/Interfaces/ITranslationService.cs)
- **Role**: load and resolve multilingual translations.
- **Endpoints**:
  - `GET {API}/trans` → list of `GLLanguage`.
  - `GET {API}/trans/{code}` → list of `GLTranslation` for the language.
- **Default language**: `en` (constant `GameConstants.DEFAULT_LANGUAGE`).
- **Activation**: gated by the config `GoodLoss.translations == "yes"`.

### 3.3 `IServiceManager` — [Interfaces/IServiceManager.cs](GLMod/Services/Interfaces/IServiceManager.cs)
...
### 3.4 `IGameStateManager` — [Interfaces/IGameStateManager.cs](GLMod/Services/Interfaces/IGameStateManager.cs)
...
### 3.5 `IItemService` — [Interfaces/IItemService.cs](GLMod/Services/Interfaces/IItemService.cs)
...
### 3.6 `IRankService` — [Interfaces/IRankService.cs](GLMod/Services/Interfaces/IRankService.cs)
- **Role**: fetch a player rank for a given mod.
- **Method**: `GetRank(modName, onComplete)` — `modName = null` ⇒ uses the current mod.

### 3.7 `IIntegrityService` — [Interfaces/IIntegrityService.cs](GLMod/Services/Interfaces/IIntegrityService.cs)
...
### 3.8 `IConfigurationService` — [Interfaces/IConfigurationService.cs](GLMod/Services/Interfaces/IConfigurationService.cs)
...
### 3.9 `IMapService` — [Interfaces/IMapService.cs](GLMod/Services/Interfaces/IMapService.cs)
...
| `GLLanguage`    | Language: code, name, translations. Has its own `load()`. |
| `GLTranslation` | (original, translation) pair. |
...
| `GoodLoss`   | `translations` | `"Yes"` loads translations at startup.               |
...
| `/trans`                       | GET    | TranslationService| List of languages.                     |
| `/trans/{code}`                | GET    | TranslationService| Translations for a language.           |
...
*Last updated: 2026-05-25 — GLMod version 5.4.0.*
